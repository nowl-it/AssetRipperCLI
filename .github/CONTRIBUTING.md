...
```bash
dotnet test AssetRipper.sln
```

### Testing CLI Mode

The project now includes CLI mode for automation. Test it using:

```bash
cd Source/AssetRipper.GUI.Free/bin/Debug/net9.0
./AssetRipper --cli -i "/path/to/test/game" -o "/tmp/test_output"
```
