...
public static class WebApplicationLauncher
{
	...
	public static void Launch(string[] args)
	{
		...
		if (args.Length > 0 && (args.Contains("--cli") || args.Contains("--input") || args.Contains("-i")))
		{
			...
			if (cliArgs.CliMode || !string.IsNullOrEmpty(cliArgs.InputPath))
			{
				CliRunner.RunCliMode(cliArgs);
				return;
			}
		}

		// Standard web interface mode
		...
		// Check if CLI mode flag is set in web arguments
		if (arguments.CliMode)
		{
			Console.WriteLine("CLI mode requested but no input specified.");
			Console.WriteLine("Use --input <path> to specify input files/directory.");
			Console.WriteLine("Run with --help for full CLI options.");
			return;
		}
